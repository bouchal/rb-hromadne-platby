...
function parseInputData(dataString) {
    ...
    if (headers.indexOf('ÚČET') === -1 || headers.indexOf('ČÁSTKA') === -1) {
        showError("Hlavička ÚČET a ČÁSTKA jsou povinné na prvním řádku")
        return
    }

    const columnIndexes = {
        bankAccount: headers.indexOf('ÚČET'),
        amount: headers.indexOf("ČÁSTKA"),
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        try {
            ...
            isError = true
            showError(`<strong>Chyba na řádku ${i}</strong><br />${e.message}`)
            break;
        }
    }
