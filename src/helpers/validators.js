...
function isAmountValid(amount) {
    // Kontrola, jestli je vstup validní číslo s možnými desetinnými místy
    if (!/^\d+(\.\d+)?$/.test(amount)) {
        return false;
    }
    ...
    // Kontrola, jestli je číslo větší než 0
    if (amountNumber <= 0) {
        return false;
    }
...
function isMessageValid(message) {
    ...
    const regex = /^[a-zA-Z0-9áčďéěíňóřšťúůýžÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ\s\.,;:()\-_\/]+$/;
